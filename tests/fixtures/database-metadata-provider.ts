...
export const DatabaseEntityMetadata: Record<string, EntityMetadata<any>> = {
	Person: {
		...
		properties: {
			...
			// 1:1 owning side — FK signature_weapon_id on persons table.
			// `inversedBy` signals ownership: the join uses this entity's FK column.
			signatureWeapon: {
				type: 'Weapon',
			...
			// 1:1 FK scalar — Person → Artifact via signature_artifact_id.
			signatureArtifactId: {
				type: 'number',
				name: 'signatureArtifactId',
				fieldNames: ['signature_artifact_id'],
				joinColumns: [],
				referencedColumnNames: [],
				inverseJoinColumns: [],
				pivotTable: '',
			...
			// 1:1 owning side — FK signature_artifact_id on persons table.
			signatureArtifact: {
				type: 'Artifact',
