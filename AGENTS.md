...
- **Numeric enum mapping** (`mapNumericEnum`): TypeScript enums (numeric or string-valued) may store raw values in the DB that differ from their GraphQL enum names. Set `mapNumericEnum: true` on a field so that **filter/input values** (string keys → raw DB values) are converted to the correct raw values for SQL `WHERE` parameters by `enum-filter-converter.ts`. On the output side, **no conversion is needed**: type-graphql registers enums with graphql-js using the raw TS enum values as the GraphQL enum values, so graphql-js's `GraphQLEnumType.serialize()` maps raw DB integers → enum names natively (e.g. `100` → `'Worthy'`). The raw column value flows through the SQL query as-is. Registry: `MapEnumFieldsMap` in `gql-entity.ts`, accessor `getMapEnumFieldsFor()`.
