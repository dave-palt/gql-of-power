...
- **Numeric enum mapping** (`mapNumericEnum`): TypeScript enums (numeric or string-valued) may store raw values in the DB that differ from their GraphQL enum names. Set `mapNumericEnum: true` on a field so that **filter/input values** (string keys → raw DB values) are converted to the correct raw values for SQL `WHERE` parameters by `enum-filter-converter.ts`. On the output side, the SQL query wraps the column in a `CASE WHEN ... THEN ... END` expression (`SQLBuilder.buildEnumCaseSQL()`) so the query returns the enum string key directly — no post-query JS needed. The CASE expression is threaded through `gql-to-sql-mapper.ts` via `enumFields` (from `MapEnumFieldsMap`), mirroring the `parseJsonFields` pattern. Registry: `MapEnumFieldsMap` in `gql-entity.ts`, accessor `getMapEnumFieldsFor()`.
