...
- **Numeric enum mapping** (`mapNumericEnum`): TypeScript enums (numeric or string-valued) may store raw values in the DB that GraphQL enums cannot serialize directly. Set `mapNumericEnum: true` on a field to have the mapper generate a `CASE WHEN ... THEN ... END` expression inline in the SELECT, so the query returns the enum string key directly — no post-query JS conversion. The CASE is built from the enum's keys via `SQLBuilder.buildEnumCaseSQL()` and injected in `processFieldNames` (`gql-to-sql-mapper.ts`), the same hook point used for `parseJson` fields. Filter values (string keys → raw values) for SQL parameters are still converted on the input side by `enum-filter-converter.ts`. Works for nested entities automatically (each entity's own SELECT gets its CASE). Registry: `MapEnumFieldsMap` in `gql-entity.ts`, accessor `getMapEnumFieldsFor()`.
