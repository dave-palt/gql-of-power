...
- **Inline filters on relation fields (child-row filtering)**: An inline filter arg like `authors { books(filter: { title_eq: "X" }) { ... } }` adds a WHERE clause **inside the child's lateral join subquery**, filtering which child rows appear in the `books` array. ALL parents are still returned. This is distinct from **top-level** filters (`authors(filter: { books: { title_eq: "X" } })`) which generate an `EXISTS` subquery that filters which parents are returned. To filter parents based on child conditions, use top-level filters or `_exists`.
...
- **Aggregate fields**: Set `aggregateFields` on relation fields to auto-generate sum/avg/min/max fields via correlated subqueries (e.g. `{ fn: 'sum', column: 'pages', fieldName: 'totalPages' }`). Each field type depends on function: `sum`/`avg` → Float, `min`/`max` → Float. Also generates numeric filter operators (`totalPages_gt`, etc.). The `column` is the property name on the related entity (resolved to SQL column via metadata). Registry: `AggregateFieldsMap` in `gql-entity.ts`, accessor `getAggregateFieldsFor()`. Handler: `mapAggregateField()` in `gql-to-sql-mapper.ts`, `tryMapAggregateFieldFilter()` in `filter-processor.ts`.
...
- **Negation filter** (`_not`): Class-level filter operator that takes an array of filter objects, AND-combines them, and wraps the result in `NOT (...)`. Enum values inside `_not` are converted recursively (same path as `_and`/`_or`). Handler: `_not()` in `filter-processor.ts`.
- **Numeric enum mapping** (`mapNumericEnum`): TypeScript enums (numeric or string-valued) may store raw values in the DB that differ from their GraphQL enum names. Set `mapNumericEnum: true` on a field so that **filter/input values** (string keys → raw DB values) are converted to the correct raw values for SQL `WHERE` parameters by `enum-filter-converter.ts`. On the output side, the behavior depends on `mapEnumOutput`:
  - `'raw'` (default) — raw DB values pass through untouched. Works when the schema is built **live** via type-graphql's `buildSchema()`, which preserves numeric enum values so graphql-js's `serialize(0)` resolves to `"ACTIVE"` natively.
  - `'key'` — SQL wraps the column in a `CASE WHEN ... THEN 'Active' END` expression so the query returns the enum string key directly. Required when the schema is **rebuilt from SDL** (Apollo Server with a pre-generated `.graphql` file), because SDL strips numeric values and `serialize(0)` fails.

  Set via per-field `mapEnumOutput: 'key'`, global `setGlobalConfig({ mapEnumOutput: 'key' })`, or env var `GQL_OF_POWER_MAP_ENUM_OUTPUT=key`. Registry: `MapEnumFieldsMap` in `gql-entity.ts`, accessor `getMapEnumFieldsFor()`. CASE builder: `SQLBuilder.buildEnumCaseSQL()`.
...
- **Distinct flag**: Set `pagination.distinct = true` to emit `SELECT DISTINCT` on the outermost query, deduplicating final rows. Useful when relationship JOINs could multiply parent rows. Auto-generates a `distinct: Boolean` field on the entity's `PaginationInput`. Works alongside `limit`, `offset`, `orderBy`, and `_or`/`_and` filters (the UNION ALL path also gets the outer `DISTINCT`; the inner structural `select distinct *` is preserved). Also supports **nested distinct** on reference-list fields (1:m and m:m relations) — pass `distinct: true` via the field's `pagination` argument to emit `SELECT DISTINCT` inside the correlated subquery, deduplicating children before `json_agg`. The `distinct` flag is threaded through `MappingsType` → `mappingsReducer` → `_dispatchRequiredRelationMapping` → relationship handlers (`buildSubFromSQL` for 1:m/1:1, inline subquery for m:m). Implementation: `distinctKeyword` in `buildQueryAndBindingsFor()` → injected into `SQLBuilder.buildSubQuery()`, the inline UNION ALL path in `gql-to-sql-mapper.ts`, and both `RelationshipHandler.buildSubFromSQL()` / `mapManyToMany()`.
...
- **OIDC trusted-publisher registration is 2FA-gated (since Aug 2026)**: npm restricts what non-interactive tokens can do to package settings — editing the trusted-publisher config itself, maintainers, or package access on npmjs.com now requires an interactive 2FA challenge in the browser. This binds the npm registration to the exact workflow filename (`release.yml`) and environment name (`release`). Renaming `.github/workflows/release.yml` or the `environment: release` line breaks publishing until someone re-registers the trusted publisher by hand with 2FA — flag this in review before touching either.
