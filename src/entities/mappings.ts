...
export const CustomFieldsMap: Record<string, CustomFieldsSettings<any>> = {};
export const CachedTypeNames: Record<any, string> = {};
...
export function createGQLEntity<T extends Object>(
  ...
  logger.fatal("-------gqlEntityName", gqlEntityName);
  ...
  if (customFields) {
    ...
    for (const fieldName of Object.keys(customFields) as (keyof typeof opts &
      ...
      const fieldOptions =
        fieldName in customFields ? customFields[fieldName] : undefined;
  ...
  return {
    GQLEntity,
    GQLEntityFilterInput:
      GQLEntityFilterInput as any as GQLEntityFilterInputFieldType<T>,
...
type FieldParameter = Parameters<
  TypeGQLMetadataStorage["collectClassFieldMetadata"]
>[0];
  ...
  if (fieldOptions.generateFilter) {
    ...
    class GQLEntityFilterInputField {
      @Field(() => [GQLEntityFilterInputField], {
        nullable: true,
        deprecationReason:
          "this is the same as using an object with multiple values",
      })
