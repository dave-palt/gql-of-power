...
export type Fields<T> = Partial<{
  [key in string & keyof T]: NonNullable<T[key]> extends Reference<infer K>
    ? Fields<K>
    : {};
}>;
...
export type GQLEntityFilterInputFieldType<T> = {
  ...
  [key in string &
    keyof typeof ClassOperations]?: GQLEntityFilterInputFieldType<T>[];
} & {
  [Key in string & keyof T as `${Capitalize<Key>}`]: {
    [key in string &
      keyof typeof FieldOperations]?: GQLEntityFilterInputFieldType<T>[];
  };
};
