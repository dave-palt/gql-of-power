...
9. **`_or` compiles to UNION ALL by default; `orStrategy: 'or'` switches to plain OR.** Each `_or` branch normally becomes a separate SELECT. Pass `pagination: { orStrategy: 'or' }` (or `setGlobalConfig({ orStrategy: 'or' })` / env `GQL_OF_POWER_OR_STRATEGY=or`) to flatten branches into one `((w1) or (w2))` WHERE — index-friendly. **Per-branch (v1.11+):** a relation field's own `pagination.orStrategy` scopes the strategy to that sub-branch (nearest-wins inheritance, siblings isolated); it's a native `PaginationInput` field, so GraphQL clients can use it directly. An explicit `buildQueryAndBindingsFor({ orStrategy })` / manager trailing param is a query-wide hard override ignoring root AND child pagination values (warns per override, `D3GOP_LOG_TYPE=orStrategy`). Both modes are verified equivalent by integration tests, including relationship-based branches (they compile to self-contained EXISTS subqueries) and count/aggregate subqueries (they dedupe on child PKs). Inline scalar `_or` inside a lateral `json_agg` is always plain OR (dedup invariant). See `references/advanced-features.md` → "orStrategy" and the README's "OR Strategy" section.
